...
import { canUseWhiteLabelPlan, fetchWorkspaceBrandingOrDefault } from "@/lib/agency"
import {
...
export async function getPublicWidgetConfig(avatarId: string, request: Request): Promise<WidgetPublicConfig> {
  ...
  const [settings, branding, billingAccount] = await Promise.all([
    ...
    fetchWorkspaceBrandingOrDefault(avatar.workspaceId),
  ...
  if (params.outputMode === "video") {
    if (params.runtimeResponse.video && params.runtimeResponse.video.status === "completed") {
      ...
      if (params.runtimeResponse.video.videoBase64) {
        ...
        videoErrorMessage = "Video was generated by the provider but is not available through controlled widget storage."
        await createRuntimeTrace({
          workspaceId: params.workspaceId,
          avatarId: params.avatarId,
          conversationId: params.conversationId,
          ...
          metadata: {
            provider: params.runtimeResponse.video.provider,
            storage: "provider_hosted_reference_blocked"
          }
        })
      } else {
...
export async function processWidgetMessage(avatarId: string, request: Request, body: unknown): Promise<WidgetMessageResponse> {
  ...
  await assertRateLimit(rateLimitPolicies.widgetMessage, [
    avatarId,
    visitorId,
    domainAccess.domain,
